...
def test_render_trial_input_promotes_amsel_priority_to_vtst_rates(tmp_path):
    script = _load_script()
    ...
    config.optionxform = str
    config.read_string(text)
    assert config["RateConstant"]["style"] == "amsel-vtst"
    assert config["RateConstant"]["compute_vineyard_prefactor"] == "True"
    ...
    assert rows == [
        {
            ...
            "seed": 1000,
            "temperature_K": None,
            "transport_lattice_parameter_A": None,
            "transport_diffusivity_A2_per_ps": None,
            "transport_alpha": None,
            "box_volume_A3": None,
            "recombined": False,
            "t_recombination_s": None,
